...
std::map<casadi_int, std::shared_ptr<JuliaOptiCallback>>& opti_callback_registry()
{
  static std::map<casadi_int, std::shared_ptr<JuliaOptiCallback>> registry;
  return registry;
}

std::mutex& opti_callback_registry_mutex()
{
  static std::mutex mutex;
  return mutex;
}

casadi_int opti_registry_key(const Opti& opti)
{
  return opti.__hash__();
}
...
void clear_opti_callback(Opti& opti)
{
  opti.callback_class();
  std::lock_guard<std::mutex> lock(opti_callback_registry_mutex());
  opti_callback_registry().erase(opti_registry_key(opti));
}
...
void register_opti_bindings(jlcxx::Module& mod)
{
  ...
  mod.method(raw_method("opti_callback"), &set_opti_callback);
  mod.method(raw_method("opti_callback_clear"), &clear_opti_callback);
