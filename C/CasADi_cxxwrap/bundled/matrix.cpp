...
std::size_t checked_size_product(const casadi_int rows, const casadi_int cols, const char* name)
{
...
casadi_int checked_auto_axis(const std::int64_t axis, const char* name)
{
  const auto converted = checked_casadi_int(axis, name);
  if(converted < -1 || converted > 1)
  {
    throw std::out_of_range(std::string(name) + " must be -1, 0, or 1");
  }
  return converted;
}

casadi_int checked_cross_dimension(const std::int64_t dim)
{
  const auto converted = checked_casadi_int(dim, "dim");
  if(converted != -1 && converted != 1 && converted != 2)
  {
    throw std::out_of_range("dim must be -1, 1, or 2");
  }
  return converted;
}
...
T dense_from_vector(jlcxx::ArrayRef<double> values, const std::int64_t rows, const std::int64_t cols)
{
  ...
  const auto expected = checked_size_product(r, c, "rows * cols");
...
template<typename T>
std::vector<T> hessian_with_gradient(const T& expression, const T& variable)
{
  T gradient;
  T hessian_result = hessian(expression, variable, gradient);
  return {hessian_result, gradient};
}
...
void register_matrix_common(jlcxx::Module& mod, const std::string& prefix)
{
  ...
  mod.method(raw_method(prefix, "cumsum"), [](const T& value, const std::int64_t axis) {
    return cumsum(value, checked_auto_axis(axis, "axis"));
  });
  mod.method(raw_method(prefix, "diff"), [](const T& value, const std::int64_t n, const std::int64_t axis) {
    return diff(value, checked_positive(n, "n"), checked_auto_axis(axis, "axis"));
  });
  ...
  mod.method(raw_method(prefix, "cross"), [](const T& lhs, const T& rhs, const std::int64_t dim) {
    return cross(lhs, rhs, checked_cross_dimension(dim));
  });
...
void register_calculus(jlcxx::Module& mod, const std::string& prefix)
{
  ...
  mod.method(raw_method(prefix, "hessian_with_gradient"), &hessian_with_gradient<T>);
